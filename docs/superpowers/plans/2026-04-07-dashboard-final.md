# Dashboard Copa do Mundo — Final Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Reescrever `dashboard.py` do zero com base no PRD aprovado — página única, 3 blocos narrativos, tema profissional claro/escuro, zero bugs.

**Architecture:** Arquivo único `dashboard.py`. `load_data()` com cache lê e limpa os 3 CSVs. `get_colors(dark)` retorna tokens de tema. `inject_css(c)` aplica CSS global. Uma função por bloco de conteúdo. `main()` orquestra tudo com `st.session_state` para dark mode.

**Tech Stack:** Python 3 · streamlit>=1.32 · pandas>=2.0 · plotly>=5.18

**Decisão de segurança:** Todos os gráficos usam `color_discrete_sequence=["#hex"]` (cor sólida) em vez de `color_continuous_scale` — evita ValueError quando dataset tem poucos valores únicos.
...
| Arquivo | Ação |
|---|---|
| `dashboard.py` | Reescrita completa |
| `requirements.txt` | Já existe, não alterar |
| `.streamlit/config.toml` | Já existe, não alterar |

---

## Task 1: Base — load_data, get_colors, inject_css

**Files:**
- Modify: `dashboard.py` (reescrever todo o conteúdo)

- [ ] **Step 1: Substituir TODO o conteúdo de `dashboard.py` por:**
...
head -5 dashboard.py
```
Esperado: `import streamlit as st`

---

## Task 2: render_header

**Files:**
- Modify: `dashboard.py` (append ao final)

- [ ] **Step 1: Adicionar render_header() ao final do arquivo**
...
## Task 3: render_bloco1 — A Evolução do Futebol

**Files:**
- Modify: `dashboard.py` (append ao final)

- [ ] **Step 1: Adicionar render_bloco1() ao final do arquivo**
...
def render_bloco1(df_cups, c: dict):
    ...
    # --- Gráfico 1: Gols por edição (área) ---
    fig1 = go.Figure()
    ...
    # --- Gráfico 2: Público por edição (barras) ---
    fig2 = px.bar(
        df_cups,
        x="Year",
        y="Attendance",
...
## Task 4: render_bloco2 — Quem Dominou a Copa

**Files:**
- Modify: `dashboard.py` (append ao final)

- [ ] **Step 1: Adicionar render_bloco2() ao final do arquivo**
...
## Task 5: render_bloco3 — Números que Surpreendem

**Files:**
- Modify: `dashboard.py` (append ao final)

- [ ] **Step 1: Adicionar render_bloco3() ao final do arquivo**
...
## Task 6: render_footer + main + entry point

**Files:**
- Modify: `dashboard.py` (append ao final)

- [ ] **Step 1: Adicionar render_footer() + main() ao final do arquivo**
...
def render_footer(c: dict):
    ...
    st.markdown(
        ...
        <div style="text-align:center; padding: 8px 0 24px 0;">
            <p style="font-size:0.8rem; color:{c['muted']}; margin:0;">
                <strong style="color:{c['text']};">Sobre o Projeto</strong>
                &nbsp;·&nbsp;
                Dados: <a href="https://www.kaggle.com/datasets/abecklas/fifa-world-cup"
                    target="_blank" style="color:#0ea5e9;">Kaggle — FIFA World Cup Dataset</a>
                &nbsp;·&nbsp; Período: 1930–2014
            </p>
            <p style="font-size:0.8rem; color:{c['muted']}; margin:6px 0 0 0;">
...
- [ ] **Step 2: Contar linhas do arquivo para confirmar que está completo**

```bash
wc -l dashboard.py
```
Esperado: entre 280 e 350 linhas.

- [ ] **Step 3: Commit**

```bash
git add dashboard.py
git commit -m "feat: dashboard interativo Copa do Mundo FIFA

- Página única com 3 blocos narrativos
- Tema claro/escuro com toggle
- CSS responsivo para mobile
- Zero color_continuous_scale (evita ValueError)
- Baseado no PRD aprovado"
```
